...
import esample.calcio.conceptualMap.objects.nodes.staffSupporto
...
val linkSSC: Link = LinkUnidirectional(
    a = staffSupporto,
    influenceType = LinkTypeImpl.DIRETTIVA,
    communicationLvl = CommunicationLevelImpl.BASSO,
    weight = {context: WeightContext, event: PureEvent ->
        if((event.personGenerated!!.first != staffSupporto && event.personGenerated!!.second != calciatori) ||
            (event.personGenerated!!.second != staffSupporto && event.personGenerated!!.first != calciatori))
...
private fun internalEvent(context: WeightContext, event: PureEvent): PureEvent{
    ...
    val weight = if(event.importance == FootballEI.IMPORTANTE)
        WeighedMood(.2f+n, .15f+n, .15f+n)
    else
        WeighedMood(.15f+n, .2f+n, .2f+n)
    return PureEvent(
        event.type,
        event.importance,
        weight.update(event.statistic),
...
private fun eventBetweenCD(context: WeightContext, event: PureEvent): PureEvent{
    ...
    val weight = if(event.importance == FootballEI.IMPORTANTE){
        if(event.type == FootballET.FIDUCIA)
            WeighedMood(.2f+n, .2f+n, .2f+n)
        else
            WeighedMood(.3f+n, .3f+n, .3f+n)
    }
    else
        if(event.type == FootballET.FIDUCIA)
            WeighedMood(.3f+n, .35f+n, .4f+n)
        else
            WeighedMood(.35f+n, .4f+n, .45f+n)
    return PureEvent(
        event.type,
        event.importance,
        weight.update(event.statistic),
